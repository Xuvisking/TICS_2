...
function Inicio(){
  ...
  const EstadoAlarma = async () =>{
    try{
      ...
      if(AlarmaActiva=='true'){
        ...
        console.log('Numbero de alarma',Alarma);
        const response= await fetch(`http://52.188.69.248:4000/api/alarma/getConfirmacionApp`,{
          ...
          body:JSON.stringify({id_alarm:Alarma.id_alarm})
        });
  ...
  const postAlarma = async () =>{
    try{
      ...
      console.log(data.id_alarm.id_alarm);
  ...
  const Salir = async  () => {
    ...
    return(
      <View style={styles.screen}>
        ...
        <Text style={styles.textoMensaje}> Estado de alarma :</Text>
        { 
